    ...
    if mode != 3:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                for i in range(amount_of_figs): #counting points
                    ...
                    d_y = event.pos[1] - y[i]
                    if a == 7:    
                        if d_x>=0 and d_y>=0 and d_x<=r[i] and d_y<=r[i] and event.pos[1]<=200: #if square isn't found
                            points += 0
                            errors += 1
                            batch_errors += 1
                            batch_points += 1
                            ...
                            pygame.display.update()
                            pygame.time.wait(points_wait_time)
                        elif d_x>=0 and d_y>=0 and d_x<=r[i] and d_y<=r[i] and event.pos[1]>200: #if square is found
                            points += 2
                            batch_points += 1
                            x[i], y[i], r[i], v_x[i], v_y[i]=0, 0, 0, 0, 0 #hide fig
        ...
        if FPS_counter % time != 0: #if time isn't over, continue the motion
            ...
            for i in range(amount_of_figs): #drawing motion
                draw_fig(x[i], y[i], r[i], color[i], mode)
                pygame.display.update()
            pgd.rect(screen, COLORS[a], (0, 0, 400, 200)) #drawing halfs
            pgd.rect(screen, COLORS[abs(1-a)], (0, 200, 400, 400))
...
screen.blit(result_1, (100, 160))
screen.blit(result_2, (100, 200))
